...
export abstract class GenericWidgetComponent implements AfterViewInit {
  ...
  @Output()
  openPanel: EventEmitter<null> = new EventEmitter<null>();

  @Output()
  deleteWidget: EventEmitter<null> = new EventEmitter<null>();
  ...
  /**
   * flag to store wheter api is calling or not
   */
  ...
  /**
   * Filters metadata list and returns non empty and null values
   * @param metadata widget metadata having null or empty values
   * @return non empty metadata list
   */
  filterMetadataByEmptyValues(metadata, fieldId?) {
    if (!fieldId) {
      fieldId = this.widgetInfo.field;
    }
    if (
      fieldId === 'STATUS' ||
      ...
      return metadata.filter((m) => {
        return m.TEXT || m.text || m.CODE || m.code;
      });
    } else {
      return metadata;
    }
