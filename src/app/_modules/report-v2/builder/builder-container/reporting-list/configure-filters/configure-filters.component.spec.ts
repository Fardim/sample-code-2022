...
import { BlockType, ConditionOperator, Criteria, DisplayCriteria, DropDownValues, FormControlType, ReportingWidget } from '@modules/report-v2/_models/widget';
...
describe('ConfigureFiltersComponent', () => {
  ...
  it(`onChange(), should add DropDownValue in the filterApplied list if already exist`, async(() => {
    ...
    component.selectedFieldMetaData = {
      ...
      fieldDesc: 'first field 1',
      sno: 1,
      fldMetaData: { fieldId: 'MATL_GROUP', picklist: '29' } as MetadataModel,
      displayCriteria: DisplayCriteria.CODE
    }
    component.tableColumnMetaData[0].fldMetaData = { picklist: '29' } as MetadataModel;
    component.onChange(value1);
    expect(component.filterApplied[component.selectedFilter.fieldId].length).toEqual(1);
    expect(component.hierarchyFilterApplied.MATL_GROUP.length).toEqual(1);
  }));
  ...
  it('getDropDownValue(), should generate filters available on fieldId', async(() => {
    ...
    const selectedItem1 = {conditionFieldValue:['222001'],conditionFieldValueText:['INDIA'],conditionFieldId:'MATL_GROUP',fieldId:'MATL_GROUP'}
    component.tableColumnMetaData = [{fields:'MATL_GROUP',fldMetaData :{ picklist: '1', isCheckList: 'true' } as MetadataModel} as ReportingWidget];
    component.getDropDownValue(selectedItem1);
    expect(component.filterApplied.MATL_GROUP.length).toEqual(1);

    const selectedItem = {conditionFieldValue:['222001'],conditionFieldValueText:['INDIA'],conditionFieldId:'MATL_GROUP',fieldId:'MATL_GROUP',parent:['true'],child:[{}]}
    component.tableColumnMetaData = [{fields:'MATL_GROUP',fldMetaData :{ picklist: '29', isCheckList: 'false' } as MetadataModel} as ReportingWidget];
    component.getDropDownValue(selectedItem);
    expect(component.hierarchyFilterApplied.MATL_GROUP.length).toEqual(1);
  }));
  ...
  // it('ngOnInit()', () => {
  //   component.ngOnInit();
  //   const formSpy = spyOn(component,'initializeForm');
  //   const columnSpy = spyOn(component,'getColumnNames');
  //   const getUserDetails = spyOn(component,'getUserDetails')
  ...
  it('applyFilter(),apply filter when click on apply button', () => {
    component.widgetId = 3743784;
    spyOn(reportService, 'setColumnMetaData').withArgs(component.tableColumnMetaData,component.widgetId);
    ...
    component.filterCriteria = [
      {
        ...
        conditionFieldStartValue: null,
        conditionFieldEndValue: null,
        conditionFieldValueText: ['test', 'test1'],
        parent:['true','true'],
        child: [null,null],
        udrid: null,
      }
  ...
  it('getFormFieldType(), get form control type', () => {
    ...
    expect(component.getFormFieldType(component.selectedFilter.fieldId)).toBeFalse();
    component.tableColumnMetaData[0].fldMetaData = { picklist: '29' } as MetadataModel;
    expect(component.getFormFieldType(component.selectedFilter.fieldId)).toEqual(FormControlType.HIERARCHY);
  })
  ...
  it('removedSelectedHierarchyFilter(), should remove hierarchy data', async()=>{
    component.selectedFilter = {fieldId:'MAT_LOC'} as Criteria;
    component.hierarchyFilterApplied = {MAT_LOC:[{CODE:'1001',TEXT:'INDIA',parent:'true'}]};
    component.filterApplied = {MAT_LOC:[{CODE:'1001',TEXT:'INDIA',parent:'true'},{CODE:'1002',TEXT:'Delhi',parent:'true'}]}
    component.filteredCriteriaList = [{fieldId:'MAT_LOC',conditionFieldValue:'1001'} as Criteria,{fieldId:'MAT_LOC',conditionFieldValue:'1002'} as Criteria]
    component.filterCriteria = [{fieldId:'MAT_LOC',conditionFieldValue:['1001','1002'],parent:['true','false'],conditionFieldValueText:['INDIA','Delhi'],child:[{CODE:'11101'}]} as any]
    const value = {CODE:'1001',TEXT:'INDIA',parent:'true',child:[{nodeId:'1002',nodeDesc:'Delhi',parent:'false',child:null}]};
    component.removedSelectedHierarchyFilter(value,0);
    expect(component.hierarchyFilterApplied.MAT_LOC.length).toEqual(0);
    expect(component.filterApplied.MAT_LOC.length).toEqual(0);
  });
});