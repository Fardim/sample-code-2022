...
import { ConditionOperator, Criteria, DisplayCriteria, ReportingWidget, OutputFormat, FormControlType, BlockType } from '@modules/report-v2/_models/widget';
...
export class ConfigureFiltersComponent implements OnInit, OnDestroy {
  ...
  getColumnNames() {
      ...
      filteredCriteriaList.forEach(item => {
        ...
        if (type === FormControlType.MULTI_SELECT) {
          ...
          if (filteredIndex > -1) {
            ...
            this.filterCriteria[filteredIndex].parent.push(parentBool);
            this.filterCriteria[filteredIndex].child.push(parentChild);
          } else {
            const parentChild = item.child ? item.child : null;
            this.filterCriteria.push({ ...item, conditionFieldValue: [item.conditionFieldValue], conditionFieldValueText: [item.conditionFieldValueText], parent: [item.parent], child: [parentChild] })
          }
    ...
    this.filterCriteria.forEach((item) => {
      if (item.conditionFieldValue) {
        const type = this.getFormFieldType(item.fieldId)
        if (type === FormControlType.MULTI_SELECT || type === FormControlType.HIERARCHY || type === FormControlType.RADIO || type === FormControlType.CHECKBOX || type === FormControlType.DROP_DOWN) {
   ...
   */
  onChange(value) {
    this.hierarchyFilterApplied[this.selectedFilter.fieldId] = [];
    ...
    if (this.filterApplied && this.filterApplied[this.selectedFilter.fieldId] && this.filterApplied[this.selectedFilter.fieldId].length) {
      if (formFieldType === FormControlType.MULTI_SELECT) {
        ...
        if(!value.length){
          this.filterApplied[this.selectedFilter.fieldId] = [];
        }
      } else if(formFieldType === FormControlType.HIERARCHY){
        this.filterApplied[this.selectedFilter.fieldId] = [];
        value.forEach(item => {
          this.filterApplied[this.selectedFilter.fieldId].push(item);
        })
        this.filterApplied[this.selectedFilter.fieldId].forEach(hieData =>{
          if(hieData.parent === 'true') {
            this.hierarchyFilterApplied[this.selectedFilter.fieldId].push(hieData);
          }
        });
        if(!value.length){
          this.filterApplied[this.selectedFilter.fieldId] = [];
      ...
      if (formFieldType === FormControlType.MULTI_SELECT) {
        ...
        value.forEach(val =>{
          if(val.parent === 'true'){
            const indexHie = this.hierarchyFilterApplied[this.selectedFilter.fieldId].findIndex(item => item.CODE === val.CODE);
            if(indexHie === -1){
              this.hierarchyFilterApplied[this.selectedFilter.fieldId].push(val);
            }
          }
        })
      } else {
    ...
    if (formFieldType === FormControlType.MULTI_SELECT) {
      ...
      this.filterCriteria[index].conditionFieldValue = [];
      this.filterCriteria[index].conditionFieldValueText = [];
      this.filterCriteria[index].parent = [];
      this.filterCriteria[index].child = [];
      this.filterApplied[this.selectedFilter.fieldId].forEach(item => {
        this.filterCriteria[index].conditionFieldValue.push(item.CODE);
        this.filterCriteria[index].conditionFieldValueText.push(item.TEXT);
        const parentBool = item.parent ? item.parent : 'false';
        const parentChild = item.child ? item.child : null;
        this.filterCriteria[index].child.push(parentChild);
        this.filterCriteria[index].parent.push(parentBool);
      })
    }
  ...
  getDropDownValue(selectedItem: any) {
    ...
    if (formFieldType === FormControlType.MULTI_SELECT) {
      ...
      this.hierarchyFilterApplied[selectedItem.fieldId] = [];
      selectedItem.conditionFieldValue.forEach((el, index) => {
        const data = { CODE: el, TEXT: selectedItem.conditionFieldValueText[index], parent: selectedItem.parent[index] ? selectedItem.parent[index] : 'false', child: selectedItem.child[index] };
        this.filterApplied[selectedItem.fieldId] ? this.filterApplied[selectedItem.fieldId].push({ ...data }) : this.filterApplied[selectedItem.fieldId] = [{ ...data }];
        if(selectedItem.parent[index] === 'true'){
          this.hierarchyFilterApplied[selectedItem.fieldId] ? this.hierarchyFilterApplied[selectedItem.fieldId].push({ ...data }) : this.hierarchyFilterApplied[selectedItem.fieldId] = [{ ...data }];
        }
      })
    } else {
  ...
  applyFilter() {
    ...
    this.filterCriteria.forEach(item => {
      ...
      if (item.conditionOperator) {
        if(formFieldType === FormControlType.HIERARCHY){
          item.conditionFieldValue.forEach((el, index) => {
            ...
            filteredCriteria.parent = item.parent[index];
            filteredCriteria.child = item.child[index];
            filteredCriteriaList.push(filteredCriteria);
          })
        }
        else if (formFieldType !== FormControlType.MULTI_SELECT) {
          const filteredCriteria = new Criteria();
          filteredCriteria.fieldId = item.fieldId;
          filteredCriteria.conditionFieldId = item.fieldId;
          filteredCriteria.blockType = BlockType.COND;
   ...
   * @param index index of lib chip
   */
  removedSelectedHierarchyFilter(code: any,index: number){
    this.filterApplied[this.selectedFilter.fieldId] = this.filterApplied[this.selectedFilter.fieldId].filter(el => el.CODE !== code.CODE);
    const ind = this.filteredCriteriaList.findIndex(item => this.selectedFilter.fieldId === item.fieldId && item.conditionFieldValue === code.CODE);
    this.filteredCriteriaList.splice(ind, 1);
    const i = this.filterCriteria.findIndex(item => item.fieldId === this.selectedFilter.fieldId && item.conditionFieldValue.indexOf(code.CODE) > -1)
    if (i > -1) {
      const conditionFieldIndex = this.filterCriteria[i].conditionFieldValue.indexOf(code.CODE);
      this.filterCriteria[i].conditionFieldValue.splice(conditionFieldIndex, 1);
      this.filterCriteria[i].conditionFieldValueText.splice(conditionFieldIndex,1);
      this.filterCriteria[i].parent.splice(conditionFieldIndex,1);
      if(this.filterCriteria[i].child){
        this.filterCriteria[i].child.splice(conditionFieldIndex,1);
      }
      if(code.parent === 'true'){
        this.hierarchyFilterApplied[this.selectedFilter.fieldId] = this.hierarchyFilterApplied[this.selectedFilter.fieldId].filter(el => el.CODE !== code.CODE);
      }
    }
    if(code.child && code.child.length){
      this.removeSelectedHierachyChild(code.child);
    }
  }
  ...
  removeSelectedHierachyChild(childData){
    childData.forEach(itemChild => {
      this.filterApplied[this.selectedFilter.fieldId] = this.filterApplied[this.selectedFilter.fieldId].filter(el => el.CODE !== itemChild.nodeId);
      const ind = this.filteredCriteriaList.findIndex(item => this.selectedFilter.fieldId === item.fieldId && item.conditionFieldValue === itemChild.nodeId);
      this.filteredCriteriaList.splice(ind, 1);
      const i = this.filterCriteria.findIndex(item => item.fieldId === this.selectedFilter.fieldId && item.conditionFieldValue.indexOf(itemChild.nodeId) > -1)
      if (i > -1) {
        const conditionFieldIndex = this.filterCriteria[i].conditionFieldValue.indexOf(itemChild.nodeId);
        this.filterCriteria[i].conditionFieldValue.splice(conditionFieldIndex, 1);
        this.filterCriteria[i].conditionFieldValueText.splice(conditionFieldIndex,1);
        this.filterCriteria[i].parent.splice(conditionFieldIndex,1);
        if(this.filterCriteria[i].child){
