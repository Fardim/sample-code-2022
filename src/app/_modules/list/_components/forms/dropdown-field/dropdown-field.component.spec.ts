...
describe('DropdownFieldComponent', () => {
  ...
  beforeEach(() => {
    ...
    // spyOn(listService, 'getFieldProperties').and.returnValues(of(dropdownFieldData));
    spyOn(coreService, 'getFieldDetails').and.returnValues(of(mochFieldList));
    // (component as any)._fieldlistContainer = mochFieldList;
  });
  ...
  it('createTextFormGroup() with data', () => {
    const data = {
      ...
      isStyledDropdown: true,
      helpText: 'helpText',
      key: true,
      tabCriteria: true,
      workflowCriteria: true,
      numberSettingCriteria: true,
      search: true,
      workflowReference: true,
      permission: true,
      transient: true,
    };
    component.createDropdownFormGroup(data);
