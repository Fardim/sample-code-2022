...
export class FormTextAreaComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes.value && changes.value.previousValue !== undefined && changes.value.previousValue !== changes.value.currentValue) {
      if (this.isFilterWidget || this.isTableFilter === 'false')
        this.control.setValue(changes.value.currentValue);
      else {
        this.textCtrl.setValue(changes.value.currentValue);
      }
    }

    if (changes.widgetId && changes.widgetId.previousValue !== undefined && changes.widgetId.previousValue !== changes.widgetId.currentValue) {
      this.widgetId = changes.widgetId.currentValue;
      this.control.setValue(changes.controls.currentValue);
    }
  }
