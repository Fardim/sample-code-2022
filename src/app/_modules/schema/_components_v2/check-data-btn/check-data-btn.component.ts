...
@Component({
  selector: 'pros-check-data-btn',
  templateUrl: './check-data-btn.component.html',
  styleUrls: ['./check-data-btn.component.scss']
})
export class CheckDataBtnComponent implements OnInit, OnDestroy {
   ...
   */
   ...
   getObjectTypes() {
     ...
     const sub = this.schemaService.getDatasetsAlongWithSchemas().subscribe(res=>{
      this.modulesList = res ? res : [];
      this.filteredModulesList = res ? res : [];
   ...
   schemaList(objectId) {
    ...
    //  const schms = this.schemas.filter(f=> f.moduleId === objectId)[0];
     if(mm) {
      ...
      this.schemaLists = mm.schemaList ? mm.schemaList  : [];
      this.filteredSchemaList = mm.schemaList ? mm.schemaList : [];
     }
   }
     ...
     if (this.data.objectid) {
       ...
       if (schemaId) {
         ...
         this.schemaService.createUpdateSchema(schemaReq).subscribe((response) => {
           ...
           this.schemaService.setRefreshSchemaList(true);
  ...
  selected(moduleId: string) {
   ...
   searchSchema(searchTerm: string) {
     if (this.schemaLists && this.schemaLists.length > 0) {
       this.searchFilter(this.schemaLists, searchTerm, 'schemaDesc')
       .subscribe((filteredresult) => {
  ...
  importSchema(type?) {
    if (!type) {
      this.router.navigate([{ outlets: { sb: `sb/schema/check-data/${this.selectedModuleId}/new` } }])
    } else {
      this.importFrmCKH();
    }
  }
